...
public class FileLoader {
    ...
    public List<Schoolarship> getDataFromJson(String filename)
    {
        try {
            ...
            BufferedReader reader = new BufferedReader(new FileReader(filename));
            //Handling data from file
            Schoolarship[] schoolarshipsArray = gson.fromJson(reader, Schoolarship[].class);
            for (int i = 0; i < schoolarshipsArray.length; i++)
            {
                schoolarships.add(schoolarshipsArray[i]);
            }
            return schoolarships;
        }catch (Exception ex)
        {
